...
// Define the Stripe secret key using Firebase params
...
function getStripeInstance(): Stripe {
  ...
  if (!stripe) {
    ...
    if (!secretVal) {
      ...
      // This error will be caught by the caller and should result in an HttpsError
      throw new Error(
...
export const createOrRetrieveStripeCustomer = onCall(
  ...
  async (request) => {
    ...
    logger.info(`[Stripe] Request auth type: ${typeof request.auth}, value:`, request.auth);

    const auth = request.auth;
    if (!auth) {
      logger.warn("[Stripe] Unauthenticated request to createOrRetrieveStripeCustomer.");
    ...
    logger.info(`[Stripe] Authenticated user UID: ${uid}, Email from token: ${email}`);

    if (!email) {
      logger.error(`[Stripe] No email on authenticated user ${uid}. Cannot create Stripe customer.`);
      throw new HttpsError(
        "internal", // Or "failed-precondition"
        "Authenticated user missing email, cannot create Stripe customer."
      );
    }
    const name = auth.token.name || email; // Default to email if name isn't in token
    ...
    try {
      ...
      logger.error(`[Stripe] Firestore error getting user ${uid}: ${errorMessage}`, dbErr);
      throw new HttpsError(
        "internal",
    ...
    if (!userSnap.exists) {
      logger.error(`[Stripe] User document for UID ${uid} not found in Firestore. This should not happen if user is authenticated and profile was created.`);
      // This case might indicate an issue with user profile creation flow.
      throw new HttpsError(
        "not-found",
        "User document not found. Cannot link Stripe customer. Please ensure your profile is complete."
      );
    }
    ...
    if (userData?.stripeCustomerId) {
      logger.info(`[Stripe] Found existing Stripe Customer ID: ${userData.stripeCustomerId} for user ${uid}. Verifying customer...`);
      try {
        const inst = getStripeInstance();
        logger.info(`[Stripe] Attempting to retrieve customer ${userData.stripeCustomerId} from Stripe.`);
        const customer = await inst.customers.retrieve(userData.stripeCustomerId);
        logger.info(`[Stripe] Stripe customer retrieved:`, customer);
        if (customer && !customer.deleted) {
          logger.info(`[Stripe] Verified Stripe Customer ID ${userData.stripeCustomerId}. Returning existing ID.`);
        ...
        logger.warn(
          ...
          "This might happen if the ID is invalid or network issues occurred. " +
          "A new one will be created."
        );
      }
    }

    logger.info(`[Stripe] No valid Stripe Customer ID found for user ${uid}, or existing one was invalid. Creating new Stripe customer.`);
    try {
      const inst = getStripeInstance();
      ...
      logger.info(`[Stripe] Created Stripe Customer ${customer.id} for UID ${uid}.`);
      await userRef.update({stripeCustomerId: customer.id});
      logger.info(`[Stripe] Updated Firestore user ${uid} with Stripe Customer ID ${customer.id}.`);
      return {customerId: customer.id};
    } catch (stripeErr: any) {
      const errorMessage = stripeErr.message || String(stripeErr);
      logger.error(`[Stripe] Stripe API error creating customer for UID ${uid}: ${errorMessage}`, stripeErr);
      throw new HttpsError(
        "internal",
...
export const createStripeSetupIntent = onCall(
  ...
  async (request) => {
    ...
    const auth = request.auth;
    if (!auth) {
      logger.warn("[Stripe] Unauthenticated request to createStripeSetupIntent.");
      throw new HttpsError(
        "unauthenticated",
    ...
    if (!customerId || typeof customerId !== "string") {
      logger.error(`[Stripe] Invalid or missing customerId in request data. CustomerId: ${customerId}, Type: ${typeof customerId}`);
      throw new HttpsError(
        "invalid-argument",
    ...
    try {
      ...
      logger.info(`[Stripe] Created SetupIntent ${setupIntent.id} for customer ${customerId}.`);

      if (!setupIntent.client_secret) {
        logger.error(`[Stripe] SetupIntent ${setupIntent.id} is missing client_secret! This is unexpected.`);
        throw new HttpsError(
          "internal",
          "Failed to obtain client_secret for SetupIntent. This should not happen."
        );
      }
      logger.info(`[Stripe] Returning client_secret for SetupIntent ${setupIntent.id}.`);
      return {clientSecret: setupIntent.client_secret};
    } catch (stripeErr: any) {
      const errorMessage = stripeErr.message || String(stripeErr);
      logger.error(`[Stripe] Stripe API error creating SetupIntent for customer ${customerId}: ${errorMessage}`, stripeErr);
      throw new HttpsError(
        "internal",
