  ...
  async (request) => {
    ...
    try {
      ...
      if (userData?.stripeCustomerId) {
        logger.info(
          `[createOrRetrieveStripeCustomer] Found existing Stripe customer ID for user ${uid}: ${userData.stripeCustomerId}`
        );
        return {customerId: userData.stripeCustomerId};
      }

      logger.info(
        `[createOrRetrieveStripeCustomer] No Stripe customer ID found for user ${uid}. Creating a new one.`
      );
      ...
      logger.info(
        `[createOrRetrieveStripeCustomer] Successfully created and saved new Stripe customer ID for user ${uid}: ${customer.id}`
      );
...
export const createStripeSetupIntent = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      logger.info(
        `[createStripeSetupIntent] Successfully created SetupIntent for customer ${customerId}.`
      );

      return {clientSecret: setupIntent.client_secret};
    } catch (error) {
      logger.error(
        `[createStripeSetupIntent] Error creating SetupIntent for customer ${customerId}:`,
        error
      );
 ...
 */
export const listStripePaymentMethods = onCall({secrets: [stripeSecretKey], cors: true}, async (request) => {
  logger.info("[listStripePaymentMethods] Function execution STARTED.");

  if (!request.auth) {
    throw new HttpsError("unauthenticated", "Authentication required to list payment methods.");
  }
  ...
  try {
    ...
    if (!customer || customer.deleted) {
      throw new HttpsError("not-found", "Stripe customer not found or has been deleted.");
    }
    ...
    logger.info(`[listStripePaymentMethods] Successfully retrieved ${formattedPaymentMethods.length} payment methods for customer ${customerId}.`);

    return {paymentMethods: formattedPaymentMethods};
  } catch (error: any) {
    logger.error(`[listStripePaymentMethods] Error listing payment methods for customer ${customerId}:`, error);
